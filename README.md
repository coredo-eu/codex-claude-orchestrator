...
**This checkout is the `codeindexer` variant.** New workers require a configured
CodeIndexer HTTP endpoint on loopback. Choose `main` if you do not use it.
...
Linux. Windows is not supported. The Codex tool shell must provide an interactive
PTY and `CODEX_THREAD_ID`; this host behavior can change between Codex versions.

Clone this branch explicitly so the marketplace uses the intended runtime:

```sh
git clone --branch codeindexer https://github.com/coredo-eu/codex-claude-orchestrator.git
cd codex-claude-orchestrator
codex plugin marketplace add "$PWD"
codex plugin add codex-claude-orchestrator@codex-claude-orchestrator
```

An existing marketplace with the same name must point at this checkout. Review
its source before reinstalling. Configure the credential-free loopback
CodeIndexer entry in the Claude profile used by the worker; see
[CodeIndexer setup and execution details](docs/operations.md).
...
must use every role. See the [skill](plugins/codex-claude-orchestrator/skills/claude-pty-agents/SKILL.md) for the exact routes.
...
- Workers make local changes. Commits, pushes, deployments, and other external
  actions stay with Codex and require the user's task to authorize them.
...
- A kill switch stops new worker interactions. Recovery checks prevent another
...
- [CodeIndexer, native routing, and admission details](docs/operations.md)
...
- [Codex agent configuration](https://github.com/coredo-eu/codex-agent-config): portable instructions and optional native specialist profiles.
- [Claude agent configuration](https://github.com/coredo-eu/claude-agent-config): configuration for standalone Claude Code.
