...
**This checkout is the `codeindexer` variant.** New workers require a
configured CodeIndexer HTTP endpoint on loopback. Choose `main` if you do not
use it. The branches are separate runtime variants — install the one you
need, and do not merge them. See
[CodeIndexer, native routing, and admission details](docs/operations.md) for
the full execution reference; the
[skill](plugins/codex-claude-orchestrator/skills/claude-pty-agents/SKILL.md)
still owns the complete transport and recovery procedure.
...
interactive PTY and `CODEX_THREAD_ID`; this host behavior can change between
Codex versions.
...
An existing marketplace with the same name must point at this checkout;
review its source before reinstalling. Configure the credential-free loopback
