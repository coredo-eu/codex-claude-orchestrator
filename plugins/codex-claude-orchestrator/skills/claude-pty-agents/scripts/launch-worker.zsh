...
# A fresh UUID must never already hold a lease, and a resumed session's exact
# stale lease was reclaimed above. Other admitted workers are represented by
# active or reserved records and retain their own session-keyed leases.
[[ ! -e "$lease" && ! -L "$lease" ]] || cco_die 75 "LEASE_CONFLICT: uuid=$session_uuid lease=$lease"
