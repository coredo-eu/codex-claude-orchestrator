...
if [[ "$action" == "status" ]]; then
  if [[ ! -e "$CCO_STATE_DIR" && ! -L "$CCO_STATE_DIR" ]]; then
    print -- "Claude PTY agents: ON — busy=0/2 active=0 orphaned=0 blocked_roots=0 live_workers=0"
    exit 0
  fi
...
if [[ "$action" == "status" ]]; then
  assignment_records=("${(@f)$(cco_active_assignments)}") || \
    cco_die 70 "CLAUDE_STATUS_ASSIGNMENT_STATE_AMBIGUOUS"
  active_count=0
  busy_count=0
  orphaned_count=0
  ...
  for assignment_record in "${assignment_records[@]}"; do
    ...
    [[ -z "${active_roots[$assignment_root]:-}" ]] || \
      cco_die 70 "CLAUDE_STATUS_DUPLICATE_ROOT_ASSIGNMENT"
